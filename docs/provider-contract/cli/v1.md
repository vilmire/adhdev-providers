...
`delegatedWorkerIsolation.workerMcpDelivery` is for CLIs such as Codex whose
MCP configuration is manual and therefore has no auto-import `mcpConfig.path`.
The daemon renders the declared templates with the worker server launch and
passes each result through `flag`. `{command_json}`, `{args_json}`, and
`{env_vars_json}` are JSON-encoded values; `{serverName}` is the dedicated
worker server name and must differ from the coordinator's server name. The
session bind value is placed only in the CLI process environment. `envVarsTemplate`
forwards its variable name to the MCP child, while `shellEnvExcludeTemplate`
can prevent ordinary shell tool children from inheriting the secret.
...
- **Per-version `compatibility[]`** — maps installed agent versions to script subdirectories or declarative spec files. Different CLI versions can require different parsing.
